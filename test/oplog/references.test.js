  ...
  describe('References', async () => {
    it('creates entries with 1 references', async () => {
      ...
      for (let i = 0; i < amount; i++) {
        await log1.append(i.toString(), { pointerCount })
      }

      const values1 = await log1.values()

      strictEqual(values1[values1.length - 1].refs.length, pointerCount)
    })
    ...
    it('creates entries with 4 references', async () => {
      const amount = 32
      const pointerCount = 4
      const log2 = await Log(testIdentity, { logId: 'B' })

      for (let i = 0; i < amount; i++) {
        await log2.append(i.toString(), { pointerCount })
      }

      const values2 = await log2.values()

      strictEqual(values2[values2.length - 1].refs.length, pointerCount)
    })

    it('creates entries with 8 references', async () => {
      const amount = 64
      const pointerCount = 8
      const log3 = await Log(testIdentity, { logId: 'C' })

      for (let i = 0; i < amount; i++) {
        await log3.append(i.toString(), { pointerCount })
      }

      const values3 = await log3.values()

      strictEqual(values3[values3.length - 1].refs.length, pointerCount)
    })

    it('creates entries with 16 references', async () => {
      const amount = 64
      const pointerCount = 16
      const log4 = await Log(testIdentity, { logId: 'D' })

      for (let i = 0; i < amount; i++) {
        await log4.append(i.toString(), { pointerCount })
      }

      const values4 = await log4.values()

      strictEqual(values4[values4.length - 1].refs.length, pointerCount)
    })

    it('creates entries with 32 references', async () => {
      const amount = 64
      const pointerCount = 32
      const log4 = await Log(testIdentity, { logId: 'D' })
